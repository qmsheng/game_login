...
namespace App\Http\Controllers\_dev_turkey;
...
class IndexController extends Controller
{
    ...
    public function proload_public_tx(Request $request)
    {
        ...
        return view('login.login')
            ->with('game_url', env('CLIENT_ROOT_DEV_TURKEY'))
            ->with('loginserver_ip', env('LOGIN_SERVER_IP_DEV_TURKEY'))
            ->with('loginserver_port', env('LOGIN_SERVER_PORT_DEV_TURKEY'))
