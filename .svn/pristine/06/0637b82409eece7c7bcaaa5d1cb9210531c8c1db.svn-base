...
namespace App\Http\Controllers\_develop;
...
class IndexController extends Controller
{
    ...
    public function proload_public_tx(Request $request)
    {
        ...
        return view('login.login')
            ->with('game_url', env('CLIENT_ROOT_DEVELOP'))
            ->with('loginserver_ip', env('LOGIN_SERVER_IP_DEVELOP'))
            ->with('loginserver_port', env('LOGIN_SERVER_PORT_DEVELOP'))
