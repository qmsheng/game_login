...
namespace App\Http\Controllers\_department;
...
class IndexController extends Controller
{
    ...
    public function proload_public_tx(Request $request)
    {
        ...
        return view('login.login')
            ->with('game_url', env('CLIENT_ROOT_DEPARTMENT'))
            ->with('loginserver_ip', env('LOGIN_SERVER_IP_DEPARTMENT'))
            ->with('loginserver_port', env('LOGIN_SERVER_PORT_DEPARTMENT'))
