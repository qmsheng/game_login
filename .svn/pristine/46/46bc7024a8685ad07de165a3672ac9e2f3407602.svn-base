...
namespace App\Http\Controllers\_public_tx;
...
class IndexController extends Controller
{
    ...
    public function proload_public_tx(Request $request)
    {
        ...
        return view('login.login')
            ->with('game_url', env('CLIENT_ROOT_URL_TX'))
            ->with('loginserver_ip', env('LOGIN_SERVER_IP_TX'))
            ->with('loginserver_port', env('LOGIN_SERVER_PORT_TX'))
